...
1.  **LSTM (Time Machine) - 역사학자**
    -   과거의 데이터를 시간 순서대로 쭉 살펴봅니다. "최근 10주 동안 이런 흐름이었으니, 다음엔 이게 나오겠네?"라고 과거의 흐름(Sequence)을 읽습니다. 10주, 50주... 1000주까지 다양한 렌즈로 봅니다.
...
## 🔧 데이터 및 문제 해결 (Troubleshooting)

### ⚠️ 쉼표 함정 (Comma Trap) 해결
-   **문제**: 1,000회차부터 회차 번호나 당첨 번호에 '쉼표(1,000)'가 포함되면서, 코드가 이를 숫자로 인식하지 못해 999회차까지만 읽는 문제가 있었습니다.
-   **해결**: `lotto_predict.py`에서 데이터를 읽을 때, 무조건 **쉼표를 제거(.replace(',', ''))**한 뒤 숫자로 변환하도록 수정했습니다. 이제 1,000회차 이상의 데이터도 완벽하게 인식합니다.

### 🔢 번호 인덱스 보정
-   구글 시트 구조에 맞춰 당첨 번호를 가져오는 위치를 **B열~G열 (인덱스 1~6)**로 정확히 맞췄습니다. 이를 통해 번호가 밀리거나 잘못 읽히는 오류를 방지했습니다.

### 🛡️ M5 Mac 안정성 모드 (Stability Mode)
-   **문제**: 최신 M5 칩(MPS)에서 FP16 가속(Mixed Precision) 사용 시 연산 충돌(mps.subtract error)로 인한 강제 종료 현상이 발견되었습니다.
-   **해결**: `lotto_predict.py`가 MPS 환경에서는 **표준 정밀도(FP32)**로 작동하도록 안전장치를 걸었습니다. 속도는 여전히 빠르지만, 튕김 현상을 원천 차단했습니다.
...
# 🤖 Hybrid Sniper V5: 하이브리드 전략 엔진 (Phase 3)

이 섹션은 **Phase 3**에서 추가된 **최종 전략 및 필터링 시스템**에 대한 설명입니다.
Phase 2의 지능형 학습 엔진에 **LLM(Gemini 1.5 Pro) 전략가**를 결합하여 인간 수준의 전략적 판단을 더했습니다.

## 🌟 주요 기능 업그레이드

### 1. Tabular Feature Attention (집중력 강화)
-   **기존**: 통계 데이터를 단순히 입력받았습니다.
-   **개선 (Phase 3)**: 당첨 여부를 결정짓는 **핵심 변수(홀짝 비율, 총합 등)에만 연산력을 집중**하는 Attention 메커니즘을 추가했습니다. 불필요한 정보는 거르고 중요한 신호만 포착합니다.

### 2. cGAN Augmentation Scale-up (연습량 100배 증가)
-   **기존**: 10배 증폭 (약 1만 건 학습)
-   **개선 (Phase 3)**: **100,000건 이상**의 가상 시나리오를 생성하여 AI가 압도적인 양의 패턴을 미리 경험하게 합니다. `generator.eval()` 모드를 정확히 적용하여 생성 중 발생하던 오류를 완벽히 해결했습니다.

### 3. Gemini 1.5 Pro Integration (LLM 전략가)
-   **역할**: 딥러닝 모델이 수학적으로 뽑아낸 '상위 후보군(Elite Candidates)'을 검토하고, **최종 10개 조합**을 선별합니다.
-   **필터링 기준**:
    -   **균형(Balance)**: 번호가 한쪽으로 쏠리지 않았는가?
    -   **흐름(Flow)**: 최근 5주간 당첨 패턴과 조화를 이루는가?
    -   **심리적 과열(Overheat)**: 너무 뻔하거나 사람이 몰릴 패턴(예: 1,2,3,4,5,6)은 제외하는가?
