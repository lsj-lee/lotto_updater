...
# 🛡️ Hybrid Sniper V5: 20인 앙상블 & 유전 엔진 (Safety First Edition)

이 섹션은 **Phase 3**를 넘어선 **최종 완성형 엔진**에 대한 설명입니다.
사용자님의 M5 MacBook 하드웨어를 완벽하게 보호하면서도, 20가지 이상의 AI 모델이 집단지성을 발휘하여 최적의 번호를 찾아냅니다.

## 🌟 핵심 전략 (Core Strategy)

### 1. 20-Model Ensemble (20인의 원탁회의)
-   **다양성 확보**: LSTM, GRU, CNN (딥러닝)과 XGBoost, RandomForest, KNN (머신러닝) 등 서로 다른 20여 가지 알고리즘이 동시에 데이터를 분석합니다.
-   **만장일치 원칙**: 한두 모델의 편향된 의견이 아닌, 20개 모델의 평균 확률값을 사용하여 신뢰도를 극대화했습니다.

### 2. Genetic Algorithm (진화론적 최적화)
-   **자연 선택**: 앙상블 모델이 추천한 확률을 바탕으로 초기 1,000개의 조합을 생성합니다.
-   **500세대 진화**: 교배(Crossover)와 돌연변이(Mutation)를 통해 500세대를 거치며 열등한 조합은 도태시키고, 가장 우수한 **Top 10** 조합만 남깁니다.

### 3. Real Data Only (실전주의)
-   가상 데이터(Synthetic Data)를 전면 폐기하고, 오직 검증된 **1,212회의 실제 당첨 기록**만을 사용하여 분석의 무결성을 지켰습니다.
...
2.  **Active Cooling (숨 고르기)**: 진화 과정 중간중간 1초씩 쉬어가며(Sleep), 칩셋의 열을 식히고 안정성을 점검합니다.
3.  **Memory Clean (메모리 청소)**: 모델 학습이 끝날 때마다 `gc.collect()`를 실행하여 찌꺼기 메모리를 즉시 제거합니다.
...
> 2. 20개의 AI 모델이 순차적으로 학습하며 확률을 계산합니다.
> 3. 유전 알고리즘이 500세대를 진화하며 최적의 조합을 찾아냅니다.
> 4. 최종 선별된 **10개의 정예 조합**이 '추천번호' 시트에 기록됩니다.
