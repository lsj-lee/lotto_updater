...
### 2. cGAN Augmentation Scale-up (스마트 데이터 증강)
-   **기존**: 10배 증폭 (약 1만 건 학습)
-   **개선 (Phase 3)**: **약 21,000건 (100배)**의 고품질 가상 시나리오를 생성합니다.
    > *Note: 초기에는 10만 건을 목표로 했으나, M5 하드웨어의 발열 제어와 안정성을 위해 2만 건 수준으로 최적화했습니다. 이는 통계적 유의성을 확보하면서도 시스템 무리를 주지 않는 최적의 지점입니다.*
...
## 🛡️ 하드웨어 안전장치 (Hardware Safety First)

사용자님의 소중한 MacBook Pro M5를 보호하기 위해 다음과 같은 안전 기능을 탑재했습니다.

1.  **Resume Training (이어하기)**: 학습 중 언제든지 중단해도 됩니다. 다시 실행하면 "이전 학습 상태에서 이어서 하시겠습니까?"라고 물어보며, `best_model.pth`에서 즉시 복구합니다.
2.  **GPU Status Check**: 매 10 에포크마다 로그를 통해 시스템 상태 점검을 권장하는 메시지를 띄웁니다.
3.  **FP32 Stability**: M5 MPS 칩셋의 특성을 고려하여, 충돌을 유발하는 FP16 연산 대신 안정적인 표준 정밀도(FP32)를 강제 적용했습니다.
