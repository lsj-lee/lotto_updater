...
# 🛡️ Hybrid Sniper V5: 최종 통합 전략 엔진 (Final Integrated Engine)

이 섹션은 **Ensemble + Genetic Algorithm + Gemini LLM**이 결합된 최종 완성형 엔진에 대한 설명입니다.
사용자님의 M5 MacBook 하드웨어를 보호(Safety First)하면서, 20가지 이상의 AI 모델과 LLM이 협력하여 최적의 번호를 찾아냅니다.

## 🌟 3단계 핵심 파이프라인 (3-Stage Pipeline)

### 1. 20-Model Ensemble (집단 지성)
-   **다양성**: RandomForest(5), XGBoost(3), CatBoost(3), KNN(5), LSTM(3), GRU(3), CNN(3) 등 총 **25개의 모델**이 동시에 데이터를 분석합니다.
-   **Anti-GIGO**: 가상 데이터를 전면 배제하고, 오직 1,212회의 **실전 데이터**만 사용하여 확률을 계산합니다.

### 2. Genetic Algorithm (500세대 진화)
-   **최적화**: 앙상블 모델이 도출한 확률을 바탕으로 초기 조합을 생성하고, 교배와 돌연변이를 통해 500세대를 진화시킵니다.
-   **쿨링 시스템**: 50세대마다 **1.5초의 휴식(Cooling Pause)**을 취하며 M5 칩의 열을 식히고 안정성을 확보합니다.

### 3. Gemini 1.5 Pro Filter (전략가)
-   **최종 선별**: 유전 알고리즘이 찾아낸 상위 30개 후보군을 **Gemini 1.5 Pro**에게 전달합니다.
-   **인간적 판단**: Gemini가 '최근 흐름(Flow)', '확률적 희소성(Scarcity)', '균형(Balance)'을 분석하여 최종 **Top 10** 정예 조합을 선택합니다.

## 🛡️ 안전 제1수칙 (Safety Protocols)

사용자님의 소중한 MacBook Pro M5를 보호하기 위해 다음과 같은 안전 기능을 탑재했습니다.

1.  **자원 제한 (Core Limiting)**: 전체 CPU 코어 중 2개를 항상 남겨두어, 분석 중에도 시스템이 버벅이지 않도록 했습니다.
2.  **Memory Clean (메모리 청소)**: 모델 학습이 끝날 때마다 `gc.collect()`를 실행하여 찌꺼기 메모리를 즉시 제거합니다.
3.  **Stability Check**: 진행 과정에서 하드웨어 온도 및 안정성을 주기적으로 체크합니다.
...
```
> **실행하면 벌어지는 일:**
> 1. 구글 시트에서 실전 데이터를 가져옵니다.
> 2. 20개 이상의 AI 모델이 앙상블 학습을 수행합니다.
> 3. 유전 알고리즘이 500세대를 진화하며 최적의 조합을 찾아냅니다. (중간중간 쿨링)
> 4. Gemini 1.5 Pro가 최종 후보를 검토하여 **10개의 정예 조합**을 '추천번호' 시트에 기록합니다.
