...
class LottoOrchestrator:
    ...
    def _write_docs(self, games, candidates):
        ...
        doc = self.docs.documents().create(body={'title': f"Sniper V5 리포트 - {datetime.date.today()}"}).execute()
        content = f"1. 핵심 후보군: {candidates}\n2. 추천 조합:\n" + "\n".join([f"Scenario {i+1}: {g}" for i, g in enumerate(games)])
        self.docs.documents().batchUpdate(documentId=doc['documentId'], body={'requests': [{'insertText': {'location': {'index': 1}, 'text': content}}]}).execute()
        print(f"   ✅ 리포트: https://docs.google.com/document/d/{doc['documentId']}")
